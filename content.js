...
var imageAPI =
  "https://pixabay.com/api/?key=18855007-d05abfa1edf07b804cd453ed1&q=";
var wordList = [];
var imgURLs = [];
...
var personObserver = new MutationObserver(async function (mutations) {
  for (let mutation of mutations) {
    if (mutation.type === "childList") {
      ...
      if (
        newText.toLowerCase() !== oldText.toLowerCase() &&
        words.length == 1
      ) {
        oldText = newText;
...
async function handleCaption(caption) {
  ...
  if (partOfSpeech == true) {
    ...
    html = `
        ...
        <div class="card">
            <img class="image" src = "${imgURL}" style="width:100%"></img> 
            <div class="container">
                <h3 class = "phrase">${caption}</h3>
            </div>
        </div>
